...
> **Изменение product scope 05.09.2026:** актуальны `docs/01-product-requirements.md` и
> `docs/04-safety-and-data-integrity.md`. Основной UI — «Оставить / К удалению», без Best,
> нескольких градаций и создания выходного альбома. Удаление доступно только отдельным
> подтверждённым PhotoKit request после проверки точного списка и source revision.
> Запрет автоматического удаления и direct Photos DB writes сохраняется. Разделы ниже,
> описывающие прежний Best/publish-only workflow, являются историческим контрактом.


> Reject-first продуктовая модель этого документа заменена [`ROADMAP.md`](ROADMAP.md)
> и ADR-0006. Ограничения безопасности остаются обязательными. Явно включаемый
> пользователем cloud boundary `Codex Vision` регулируется более новым ADR-0008 и является
> единственным исключением из исторического запрета cloud image APIs.
