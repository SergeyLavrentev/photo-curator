            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                GroupBox("Что делает приложение") {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        Label("Предлагает лучшие кадры, альтернативы и повторы для проверки", systemImage: "rectangle.3.group")
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                GroupBox("Фильтры и правила отбора") {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HelpRuleRow(
                            ...
                            text: "Вы выбираете компактную, сбалансированную или широкую подборку. Цель — примерно 25%, 45% или 65% альбома. Порог зависит от распределения оценок; повторы и разнообразие могут уменьшить подборку."
                        )
                        HelpRuleRow(
                            title: "Разнообразие",
                ...
                GroupBox("Почему решение можно проверить") {
                    VStack(alignment: .leading, spacing: 10) {
                        Text("Для каждого кадра сохраняются использованные версии моделей, доступность сигналов, компоненты оценки и короткие причины. Причины объясняют качество кадра и его место в подборке отдельно. Низкий балл или несоответствие вкусу сами по себе не делают фотографию браком.")
                        Text("Если превью или анализ недоступны, кадр сохраняется для проверки — движок не исключает его на основании отсутствующих данных. Ваше ручное решение всегда имеет приоритет над автоматическим.")
                            .fontWeight(.semibold)
                    }
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(.vertical, 4)
...
private let analysisHelpStages = [
    ...
    AnalysisHelpStage(
        ...
        engine: "Локальный Python Swipe Score engine; профиль вкуса — pairwise-linear-v6 с разделением по альбомам, обученный только на ваших явных предпочтениях и совместимый с текущей схемой feature print.",
    ...
    AnalysisHelpStage(
        ...
        engine: "Python decision engine v7 и diversity engine vision-feature-diversity-v1. Семантическая близость от 0,94 и больше трёх похожих кадров ограничивают автоматическую подборку.",
        output: "Best, альтернативы и кадры для проверки; отдельно — Keep/Reject и объяснение решения."),
    AnalysisHelpStage(
        number: 8,
        title: "Ваша проверка",
        symbol: "hand.tap",
        ...
        GroupBox("Apple Vision: какие сигналы даёт система") {
            VStack(alignment: .leading, spacing: 14) {
                ...
                VisionSignalRow(
                    ...
                    details: "Определяются количество лиц, доступность landmark-точек и face capture quality. Для ограниченного shortlist похожей серии приложение повторно измеряет резкость лица, объекта и предполагаемых областей глаз на полном review render. Это относительный sharpness-сигнал, а не blink/open-eye detector; он не создаёт automatic Reject. Отсутствие лица не считается недостатком фотографии."
                )
