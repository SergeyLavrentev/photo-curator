...
сервис создаёт частичный или полный service-owned disk snapshot после подтверждения.
Photos Library не изменяется. Видео пропускаются; при отсутствии локального render UI
объясняет необходимость дождаться синхронизации.
