...
def _confirm_pair(
    ...
    *,
    ...
    exact = left.get("normalized_pixel_hash") == right.get("normalized_pixel_hash")
    ...
    strict = distance <= strict_phash_distance and aspect_delta <= 0.08
    relaxed = (
    ...
    if not (exact or strict or relaxed or burst):
        return None
...
def _build_group(
    ...
    group_kind = (
        "exact"
        if all(
            asset.get("normalized_pixel_hash") == leader.get("normalized_pixel_hash")
            for asset in assets
        )
        else "near"
    )
...
def _quality(asset: dict[str, object]) -> float:
    signals = [
        ...
        (asset.get("technical_quality"), 0.35),
    ]
